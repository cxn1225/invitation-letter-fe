  ...
  <main v-if="invite" class="invite-page">
    <div class="invite-shell">
      ...
      <section v-for="block in invite.blocks" :key="block.id" class="invite-card">
        ...
        <ol v-if="block.type === 'agenda_teaser' && invite.agenda.length" class="agenda-list">
          <li v-for="item in invite.agenda" :key="item.id" class="agenda-item">
            <time>{{ item.timeLabel }}</time>
            <div><strong>{{ item.title }}</strong><span v-if="item.detail || item.location">{{ item.detail || item.location }}</span></div>
          </li>
        </ol>
      </section>
